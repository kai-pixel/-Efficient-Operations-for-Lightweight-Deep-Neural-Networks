...
class gconv(nn.Module):
   def __init__(self, Block_num, exp_size, kernel_size, stride, BN_momentum):
       super(gconv,self).__init__()
       self.Block_num = Block_num

       self.conv = nn.Conv2d(in_channels=exp_size, out_channels=exp_size, kernel_size=kernel_size, stride=stride, padding=(kernel_size-1)//2, groups=exp_size, bias=False)
       self.bn = nn.BatchNorm2d(num_features=exp_size, momentum=BN_momentum)
       # groups=exp_size//2 or groups=exp_size//3
       self.conv1 = nn.Conv2d(in_channels=exp_size, out_channels=exp_size, kernel_size=kernel_size, stride=stride, padding=(kernel_size-1)//2, groups=exp_size//2, bias=False)
       self.bn1 = nn.BatchNorm2d(num_features=exp_size, momentum=BN_momentum)

   def forward(self, x):
     if self.Block_num==1 or self.Block_num==2 or self.Block_num==3 or self.Block_num==4 or self.Block_num==5 or self.Block_num==6 or self.Block_num==7:
...
class Bottleneck(nn.Module):
    ...
    '''
    def __init__(self, Block_num, in_channels_num, exp_size, out_channels_num, kernel_size, stride, use_SE, NL, BN_momentum):
        '''
        ...
        if exp_size == in_channels_num:
            ...
            self.conv = nn.Sequential(
                ...
                #nn.BatchNorm2d(num_features=exp_size, momentum=BN_momentum),
                gconv(Block_num, exp_size, kernel_size, stride, BN_momentum),
                # SE Module
                SEModule(exp_size) if use_SE else nn.Sequential(),
                H_swish() if use_HS else nn.ReLU(inplace=True),
...
class cheapV3G(nn.Module):
    def __init__(self, mode='large', classes_num=10, input_size=32, width_multiplier=1.0, dropout=0.2, BN_momentum=0.1, zero_gamma=False):
        ...
        if mode == 'large':
            ...
            configs = [
                #Block_num, kernel_size, exp_size, out_channels_num, use_SE, NL, stride
                [1, 3, 24, 24, False, 'RE', 1],
                [2, 3, 144, 24, False, 'RE', s],
                [3, 3, 144, 24, False, 'RE', 1],
                [4, 5, 144, 48, True, 'RE', 2],
                [5, 5, 288, 48, True, 'RE', 1],
                [6, 5, 288, 48, True, 'RE', 1],
                [7, 3, 288, 96, False, 'HS', 2],
                [8, 3, 576, 96, False, 'HS', 1],
                [9, 3, 576, 96, False, 'HS', 1],
                [10, 3, 576, 96, False, 'HS', 1],
                [11, 3, 576, 120, True, 'HS', 1],
                [12, 3, 720, 120, True, 'HS', 1],
                [13, 5, 720, 180, True, 'HS', 2],
                [14, 5, 1080, 180, True, 'HS', 1],
                [15, 5, 1080, 180, True, 'HS', 1]
            ]
        elif mode == 'small':
        ...
        for Block_num, kernel_size, exp_size, out_channels_num, use_SE, NL, stride in configs:
            ...
            feature_extraction_layers.append(Bottleneck(Block_num, input_channels_num, exp_size, output_channels_num, kernel_size, stride, use_SE, NL, BN_momentum))
